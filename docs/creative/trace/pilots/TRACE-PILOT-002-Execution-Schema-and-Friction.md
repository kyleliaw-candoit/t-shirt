...
| `P002-FRI-009` | The DTC panel was locked after the Amazon and Etsy raw captures instead of immediately after preregistration. | Preserve the actual chronology. The panel remains usable only if the audit confirms that all 12 stores were fixed before any DTC ranked-result or product-detail inspection and that earlier marketplace observations did not influence store membership. | The ranked DTC frame was not exposed before the lock. This is an execution-order deviation requiring a checkpoint no-effect finding, not authority to relax the panel-lock rule. |
| `P002-FRI-010` | Noncontrolling Etsy origin searches began before the DTC raw frame closed and before the cross-platform family-deduplication checkpoint. | Preserve the fragments under the [prestage manifest](./TRACE-PILOT-002-Noncontrolling-Prestage-Origin-Manifest.md), give them no selection or downstream credit, and rerun the controlling origin stage from each frozen queue's rank 1 after all raw frames and the dedup checkpoint are frozen. | Superseding the early work prevents it from changing final eligibility, order, identity, lineage, or support. The actual timestamps remain visible. |
| `P002-FRI-011` | The initial Etsy CSV placed each `SURFACE` note in an unintended 41st field while organic rows and the header had 40 fields. | Remove only the surplus empty field on the 12 `SURFACE` rows, retain every evidence value and note verbatim, and record the post-origin repair. | The repair changes no surface, observation, ordinal, candidate, queue rank, or disposition. All 157 rows now validate at 40 fields. |
| `P002-FRI-012` | Most prestage Etsy source-side origin runs used domain-scoped web search without first documenting that Etsy native search was unavailable. | Treat every prestage decision as noncontrolling. A controlling rerun must use Etsy native plus a separate open-web search, or record a truthful bounded native-access failure and apply the frozen unresolved-search rule. | This restores the preregistered surface rule; it does not use the defective runs to qualify or reorder a source. |
| `P002-FRI-013` | The `02-TR` and `03-MR` Etsy surfaces were authoritatively reclosed after several later-numbered surfaces, despite the raw report's phrase “immediately reclosed.” | Preserve the timestamps, replace that wording with a chronological description, and verify that no queue tie or first-occurrence decision changes. | The final complete frames and frozen query-major ordinal rules remain unchanged; no current queue tie depends on first occurrence. |
| `P002-FRI-014` | The prestage fragments used inconsistent N/A values and sometimes combined `TRACEABLE_SOURCE` enum values with explanatory text. | Do not normalize the superseded raw fragments. Use the frozen enums in the controlling register, with reasons in `NA_REASON`, `RUN_NOTES`, or `MATCH_NOTES`. | The fragments remain truthful audit evidence; only compliant post-raw records can control eligibility. |
